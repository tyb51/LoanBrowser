...
export async function compareLoans(
  ...
  if (investmentParams) {
    ...
    for (let month = 1; month <= maxMonths; month++) {
      ...
      const refMonthData = refResult.monthlyData.find(m => m.Maand === month);
      const altMonthData = altResult.monthlyData.find(m => m.Maand === month);
    ...
    const totalCosts = refResult.statistics["Totale Kosten Lening (Rente + SSV)"];
    const totalAltCosts = altResult.statistics["Totale Kosten Lening (Rente + SSV)"];
    ...
    if (investmentSimulation.length > 0) {
      ...
      comparisonStats = {
        ...
        netWorthEndOfTerm: lastMonth["Netto Vermogen (Invest - Schuld)"]
      };
    }
  }
